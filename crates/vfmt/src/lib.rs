//! A modified version of [μfmt](https://github.com/japaric/ufmt).
//!
//! Changes made:
//!
//! - Removed `proc-macro-hack`
//! - Added simple padding support
//! - Added stdout and stderr support
//! - Added a replacement [`std::format`]
//!
//! ---
//!
...
pub trait uDisplay {
    ...
    /// A replacement for [`ToString::to_string`].
    fn to_string(&self) -> String {
        use crate as vfmt;

        let mut buf = String::new();
        let _ = uwrite!(&mut buf, "{}", self);
        buf
    }
}
    ...
    W: uWrite + ?Sized,
{
    ...
    /// A adds `amount` of spaces to the right of the buffer.
    pub fn pad_right_spaces(
        &mut self,
        amount: usize,
        f: impl FnOnce(&mut Self) -> Result<(), W::Error>,
    ) -> Result<(), W::Error> {
